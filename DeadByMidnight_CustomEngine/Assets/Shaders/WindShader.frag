...
@brief      This file implements the vertex shader for OpenGL use, integrated to create Animated Wind Tunnel Textures.
...
void main () 
{      
	vec4 _WindColor = vec4(0.79f, 0.62f, 0.96f, 1.0f);
...
vec4 dummyvariable = texture(uTex2d, vTexCoord);
  ...
  if (uTextureEnabled)
  {
                vec4 c = _WindColor;
                //PART A: BODY
                float thresholdA = 0.4f;
                float thresholdB = 0.95f;
                c.a *= 0.25f * (1.f - clamp(smoothstep(thresholdA - 0.2f, thresholdA + 0.05f, vTexCoord.y + 0.5f * pow(abs(0.5f - vTexCoord.x), 2.5f)), 0.f, 1.f))
                    + 0.15f * (1.f - clamp(smoothstep(thresholdB - 0.2f, thresholdB + 0.05f, vTexCoord.y + 0.5f * pow(abs(0.5f - vTexCoord.x), 3.f)), 0.f, 1.f))
                    ;
                c.a *= (1.f - pow(clamp(smoothstep(0.2f, 0.5f, abs(0.5f - vTexCoord.x)), 0.f, 1.f), 1.4f));
                ...
                distortion -= 0.7f;
                distortion *= 0.5f;
                vec2 windCoord = vTexCoord * vec2(1.f, 2.04f) - vec2(0.f, _Time);
                ...
                contentC.a *= (1.f - clamp(smoothstep(0.4f, 0.49f, abs(0.5f - (mod((abs(vTexCoord.y - 0.49f * _Time) * 2.04f - 0.02f), 1.f)))), 0.f, 1.f));
