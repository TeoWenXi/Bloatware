/*!
All content © 2023 DigiPen Institute of Technology Singapore, all rights reserved.
@file       Matrix3x3.cpp
@author     CAI Wenjin (100%)
@co-author

----------------------------------------------------------------------------

@course     CSD2401F23
@section    CSD2401F23-A
@project    CSD2401F23 Software Engineering Project 3

@brief      Matrix 3x3 class definitions.
*//*______________________________________________________________________*/

#include "Matrix3x3.h"
#if _ENGINE
#include "DebugLogger.h"
#endif // _ENGINE



Matrix3x3::Matrix3x3()
{
	SetToIdentity();
}

Matrix3x3::Matrix3x3(float _m00, float _m10, float _m20, float _m01, float _m11, float _m21, float _m02, float _m12, float _m22)
{
	mMat[0] = _m00, mMat[1] = _m10, mMat[2] = _m20,
		mMat[3] = _m01, mMat[4] = _m11, mMat[5] = _m21,
		mMat[6] = _m02, mMat[7] = _m12, mMat[8] = _m22;
}

Matrix3x3::Matrix3x3(const Matrix3x3& _rhs)
{
	mMat = _rhs.mMat;
}

Matrix3x3& Matrix3x3::operator=(const Matrix3x3& _rhs)
{
	if (this != &_rhs)
		mMat = _rhs.mMat;

	return *this;
}

Matrix3x3::Matrix3x3(Matrix3x3&& _rhs) noexcept
{
	mMat = std::move(_rhs.mMat);
}

Matrix3x3& Matrix3x3::operator=(Matrix3x3&& _rhs) noexcept
{
	if (this != &_rhs)
		mMat = std::move(_rhs.mMat);

	return *this;
}

float& Matrix3x3::operator[](unsigned _val)
{
	if (_val > 8)
	{
		try
		{
			throw std::out_of_range("Error: attempt to access invalid index in matrix.\n");

		}
		catch (const std::exception& e)
		{
			std::cerr << e.what() << std::endl;
			return mMat[0];
		}
	}
	else
		return mMat[_val];
}

Matrix3x3::~Matrix3x3()
{
}

void Matrix3x3::SetToIdentity()
{
	mMat.fill(0.f);
	mMat[0] = mMat[4] = mMat[8] = 1.f;
}

void Matrix3x3::SetToZero()
{
	mMat.fill(0.f);
}

Vector3 Matrix3x3::GetCol(unsigned int _col) const
{
	if (_col >= 3)
	{
		try
		{
			throw std::out_of_range("Error: attempt to access invalid column in matrix.\n");

		}
		catch (const std::exception& e)
		{
			std::cerr << e.what() << std::endl;
		}

		return Vector3();
	}

	return Vector3(mMat[_col], mMat[_col + 3], mMat[_col + 6]);
}

Vector3 Matrix3x3::GetRow(unsigned int row) const
{
	if (row >= 3)
	{
		try
		{
			throw std::out_of_range("Error: attempt to access invalid row in matrix.\n");

		}
		catch (const std::exception& e)
		{
			std::cerr << e.what() << std::endl;
		}

		return Vector3();
	}

	return Vector3(mMat[row * 3], mMat[row * 3 + 1], mMat[row * 3 + 2]);
}

void Matrix3x3::Transpose()
{
	std::swap(mMat[1], mMat[3]);
	std::swap(mMat[2], mMat[6]);
	std::swap(mMat[5], mMat[7]);
}

void Matrix3x3::Inverse()
{
	float determinant =
		(mMat[0] * (mMat[4] * mMat[8] - mMat[5] * mMat[7])) -
		(mMat[1] * (mMat[3] * mMat[8] - mMat[5] * mMat[6])) +
		(mMat[2] * (mMat[3] * mMat[7] - mMat[4] * mMat[6]));

	if (determinant != 0.f)
	{
		Matrix3x3 adjugate{};

		adjugate.mMat[0] = (mMat[4] * mMat[8] - mMat[5] * mMat[7]);
		adjugate.mMat[1] = (mMat[2] * mMat[7] - mMat[1] * mMat[8]);
		adjugate.mMat[2] = (mMat[1] * mMat[5] - mMat[2] * mMat[4]);
		adjugate.mMat[3] = (mMat[5] * mMat[6] - mMat[3] * mMat[8]);
		adjugate.mMat[4] = (mMat[0] * mMat[8] - mMat[2] * mMat[6]);
		adjugate.mMat[5] = (mMat[2] * mMat[3] - mMat[0] * mMat[5]);
		adjugate.mMat[6] = (mMat[3] * mMat[7] - mMat[4] * mMat[6]);
		adjugate.mMat[7] = (mMat[1] * mMat[6] - mMat[0] * mMat[7]);
		adjugate.mMat[8] = (mMat[0] * mMat[4] - mMat[1] * mMat[3]);

		for (int i = 0; i < 9; ++i)
			mMat[i] = (1.f / determinant) * adjugate.mMat[i];
	}
	else
	{
		// need do error handling here
		// current implementation will set to identity matrix
#if _ENGINE
		DebugLogger::GetInstance()->Log("Matrix is non-invertable, setting matrix to identity.\n");
#endif // _ENGINE

		SetToIdentity();
	}
}

Matrix3x3 Matrix3x3::Translate(float _x, float _y, float _z)
{
	Matrix3x3 mat{};

	mat[6] += _x;
	mat[7] += _y;
	mat[8] += _z;

	return mat;
}

Matrix3x3 Matrix3x3::Translate(const Vector3& _vec)
{
	return Translate(_vec.X(), _vec.Y(), _vec.Z());
}

Matrix3x3 Matrix3x3::RotateDegrees(float _deg)
{
	Matrix3x3 mat{};
	mat.mMat[0] = std::cosf(DegreeToRadians(_deg)); mat.mMat[3] = -std::sinf(DegreeToRadians(_deg));
	mat.mMat[1] = std::sinf(DegreeToRadians(_deg)); mat.mMat[4] = std::cosf(DegreeToRadians(_deg));

	return mat;
}

Matrix3x3 Matrix3x3::RotateRadians(float _rad)
{
	Matrix3x3 mat{};
	mat.mMat[0] = std::cosf(_rad); mat.mMat[3] = -std::sinf(_rad);
	mat.mMat[1] = std::sinf(_rad); mat.mMat[4] = std::cosf(_rad);

	return mat;
}

Matrix3x3 Matrix3x3::Scale(float _x, float _y, float _z)
{
	Matrix3x3 mat{};
	mat.mMat[0] = _x;
	mat.mMat[4] = _y;
	mat.mMat[8] = _z;

	return mat;
}

Matrix3x3 Matrix3x3::Scale(const Vector3& _vec)
{
	return Scale(_vec.X(), _vec.Y(), _vec.Z());
}

Matrix3x3::operator glm::mat3() const
{
	return glm::mat3
	{
		mMat[0], mMat[1], mMat[2],
		mMat[3], mMat[4], mMat[5],
		mMat[6], mMat[7], mMat[8],
	};
}

Matrix3x3 operator*(const Matrix3x3& _lhs, const Matrix3x3& _rhs)
{
	Matrix3x3 result{};

	for (size_t col{}; col < 3; ++col)
	{
		for (size_t row{}; row < 3; ++row)
		{
			float sum = 0.f;

			// perform the dot product of the column from lhs and the row from rhs.
			for (size_t i{}; i < 3; ++i)
				sum += _lhs.mMat[i * 3 + col] * _rhs.mMat[row * 3 + i];

			result.mMat[row * 3 + col] = sum;
		}
	}

	return result;
}

Vector3 operator*(const Matrix3x3& _lhs, const Vector3& _rhs)
{
	Vector3 result{};
	result.X((_lhs.mMat[0] * _rhs.X()) + (_lhs.mMat[3] * _rhs.Y()) + (_lhs.mMat[6] * _rhs.Z()));
	result.Y((_lhs.mMat[1] * _rhs.X()) + (_lhs.mMat[4] * _rhs.Y()) + (_lhs.mMat[7] * _rhs.Z()));
	result.Z((_lhs.mMat[2] * _rhs.X()) + (_lhs.mMat[5] * _rhs.Y()) + (_lhs.mMat[8] * _rhs.Z()));
	return result;
}

Matrix3x3 operator*(const Matrix3x3& _lhs, float _val)
{
	Matrix3x3 mat{};
	for (int i{}; i < 9; ++i)
		mat.mMat[i] = _lhs.mMat[i] * _val;

	return mat;
}

std::ostream& operator<<(std::ostream& _os, const Matrix3x3& _mat)
{
	_os << "| " << _mat.mMat[0] << " , " << _mat.mMat[3] << " , " << _mat.mMat[6] << " |\n";
	_os << "| " << _mat.mMat[1] << " , " << _mat.mMat[4] << " , " << _mat.mMat[7] << " |\n";
	_os << "| " << _mat.mMat[2] << " , " << _mat.mMat[5] << " , " << _mat.mMat[8] << " |\n";
	return _os;
}
