...
{
	//Level loading
	LevelLoader::LoadLevel(SceneManager::DemangleSceneName(typeid(GameScene).name()),GraphicsSystem::GetInstance()->mBackgroundColor);
...
void GameScene::Input(double _dt)
{
	...
	if (InputManager::GetInstance()->GetActionTriggered("CheatSkip")
		&& mCamWaypointIndex < ((int)mCamWaypoints.size() - 2)
		&& EntityManager::GetInstance()->GetEntity(Player::mPlayerID)->GetComponent<Player>()->mCutsceneTimer <= 0)
	{
		//Move camera
		GraphicsSystem::GetInstance()->mCamera.mCurrCamInfo = mCamWaypoints[mCamWaypoints.size() - 2];
		mCamWaypointIndex = (int)mCamWaypoints.size() - 2;
		...
		for (auto& obj : csScriptArr)
		{
			...
			if (entName.find("Parallax") != std::string::npos)
				EntityManager::GetInstance()->GetEntity(obj.first)->GetComponent<Transform>()->SetPosition(mCamWaypoints[mCamWaypoints.size() - 2].mPosition);
		}
	}
}

void GameScene::Update(double _dt)
{
	UNREFERENCED_PARAMETER(_dt);
...
void GameScene::Pause(double _dt)
{
	UNREFERENCED_PARAMETER(_dt);
	Entity* player = EntityManager::GetInstance()->GetEntity(Player::mPlayerID);
	if (player != nullptr)
...
void GameScene::Render(double _dt)
{
	UNREFERENCED_PARAMETER(_dt);
